...
- **クライアント → サーバー**: `JOIN_REQUEST`（1 バイト） — 「マッチに入れてほしい」
...
**接続はプレイヤーではありません。** クライアントはプロセス起動時に ENet ピアを開き、
プロセス終了まで保持し続けますが、その大半の時間はメニュー画面です。したがって接続直後の
ピアは観戦者として扱います。同じワールドを読み込んだか検証できるだけの `MapInfo` を送り、
それ以外は何もしません。`JOIN_REQUEST` を受け取るまでスポーンさせず、スナップショットも
送らず、`MatchConfig::MIN_PLAYERS` にも数えません。この分離がないと、タイトル画面で放置
された複数のクライアントだけで人数ゲートが満たされ、誰もいないマップで 60 秒のマッチが
最後まで進行してしまいます。
...
`tools/run_sessiontest.sh` はサーバーのライフサイクル適合性スイートです。ポートを確保
できなかったサーバーが「ソケットなしで黙って動き続ける」のではなく非ゼロで終了すること、
および接続と参加の分離が保たれていること（`JOIN_REQUEST` 未送信のピアはスポーンもスナップ
ショットもなし、送信したピアはプレイヤーになる、放置ピアだけでは `MIN_PLAYERS` に到達
しない、実際に 2 人参加すればマッチは開始する）を検証します。
...
default.map / shipment.map      バイナリの隣に配置されるマップ（Docker では /app/）
Network/
...
├── session_test.cpp            ヘッドレス接続 / 参加適合性クライアント（非同梱）
├── run_sessiontest.sh          サーバーライフサイクル適合性スイート
